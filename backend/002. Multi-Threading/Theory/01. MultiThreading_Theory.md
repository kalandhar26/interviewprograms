...
## 1. Is a thread lighter than a process and can exits without it?

- A process is an instance of a program that is being executed. When a program runs, the operating system creates a
  process to manage its execution.
- A thread is the smallest unit of execution with in a process. A process can have multiple threads, which share the
  same resource but can run independently.

## When a Multiple Users hit the same REST API at the same time. Are they served by one thread or multiple?

- They are served by multiple threads. When many users hit the same REST API at the same time, the server assigns a
  seperate thread for each request. so request can run in parallel, not one by one.

## Difference between extending Thread and implementing runnable interface? When to use each?

- In Extending Thread, our class becomes the thread.
- We put our code in run() method and start using start() method.
- But the limitation is that our class cannot extend any other classes because java allows only one class to be
  extended.
- usage : simple demos or learning purpose. Small programs where inheritance is not needed.
- In Implementing Runnable, our class represents task, not the thread itself.
- We put out code in run() method and pass it to a Thread Object.
- Here the class can extend another class.
- usage: In Real World applications, When our class already extends another class.

## Can a class extend Thread and Implement Runnable together?

- Yes, a class can extend Thread and implement Runnable at the same time. but it makes no real sense.
- Why? Thread Class alreading implementing Runnable interface. again if implement runnable it adds nothing.